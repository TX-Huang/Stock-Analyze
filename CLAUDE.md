...
### 已測試但不採用的策略變更（回測日期：2026-03-24）

| 變更 | 原始 | 修改後 | CAGR | MDD | 風報比 | 結論 |
|------|------|--------|------|-----|--------|------|
| **ATR 正規化部位規模** | 等權重 1/10 | 波動率反比加權 (0.3x~2x) | 21.33% → 17.91% (-3.4%) | -30.87% → -29.77% (+1.1%) | 1.85 → 1.89 | **不採用** — CAGR 下降過多，MDD 改善不顯著 |
| **流動性門檻提高** | 50 萬股 | 100 萬股 | 21.33% → 14.81% (-6.5%) | -30.87% → -37.39% (-6.5%) | 1.85 → 1.79 | **不採用** — 全面劣化 |

> **結論**：原始 Isaac V3.7 配置（等權重 + 50 萬股門檻）是目前最佳平衡點。
> ATR 邏輯保留在程式碼中作為實驗參數 `params={'atr_exit': True}`，未來可隨時重新測試。
> 回測比較腳本：`scripts/backtest_comparison.py`，結果：`data/backtest_comparison.json`

### 待測試的策略改善項目
- [ ] 訊號協調器（Signal A-E 衝突時的優先權規則）
- [ ] 時間停損（持倉 >60 天未獲利出場）`params={'time_stop_days': 60}`
- [ ] 突破確認（2 日收盤確認）`params={'confirm_days': 2}`
- [ ] 產業集中度限制 `params={'max_per_industry': 3}`
- [ ] Score 最低門檻 `params={'min_score': 5}`
- [ ] 大盤寬度過濾 `params={'breadth_filter': True}`
- [ ] 滑價模型整合（需 FinLab 支援或自建引擎）
