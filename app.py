...
if app_mode == "🧬 量化回測系統":
    ...
    if run_btn:
        if not finlab_token:
            ...
            # [BUG FIX #3] 清除舊報告，避免失敗時顯示殘留的舊結果
            st.session_state.backtest_report = None
            st.session_state.current_strategy = None

            with st.spinner(f"正在執行 {strategy_type} 回測... (這可能需要幾分鐘)"):
                try:
                    # [BUG FIX #1] 使用 importlib.reload() 強制重新載入策略模組
                    # Python 的 import 語句會快取模組，修改 .py 後不會自動重新載入
                    # 必須用 reload() 確保每次回測都使用最新的策略代碼
                    import importlib
